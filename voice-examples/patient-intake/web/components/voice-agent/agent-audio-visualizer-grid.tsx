...
import {
  ...
  type Coordinate,
} from './use-agent-audio-visualizer-grid';
...
export interface GridOptions {
  ...
  columnCount?: number;
  /**
   * A function to transform the style of each grid cell based on its position. Receives the cell
   * index, row count, and column count as arguments.
   */
...
export type AgentAudioVisualizerGridProps = GridOptions & {
  ...
  className?: string;
  /**
   * Custom children to render as grid cells. Each child receives data-lk-index and
   * data-lk-highlighted props.
   */
  children?: ReactNode;
} & VariantProps<typeof AgentAudioVisualizerGridVariants>;

/**
 * A grid-style audio visualizer that responds to agent state and audio levels. Displays an animated
 * grid of cells that react to the current agent state and audio volume when speaking.
 *
 * @example
 *
 * ```tsx
